class QuestionsController < ApplicationController
  ...
  def diagnose_from_answers(answers)
    a_count = answers.values.count { |choice_id| Choice.find(choice_id).question_type == 0 }
    b_count = answers.values.count { |choice_id| Choice.find(choice_id).question_type == 1 }
    c_count = answers.values.count { |choice_id| Choice.find(choice_id).question_type == 2 }
    ...
    if a_count >= 3 && answers.values.any? { |choice_id| Choice.find(choice_id).question_body == "36.2℃より高い" }
      ColdSymptom.symptom_types[:systemic]
    end
